...
class OmenACPIInterface:
    def __init__(self):
        ...
        # Command constants from original Windows version
        self.CMD_DEFAULT = 0x20008    # Most commands (131080)
        self.CMD_KEYBOARD = 0x20009   # Keyboard-related (131081)
        self.CMD_LEGACY = 0x00001     # Earliest implemented (1)
        self.CMD_GPU_MODE = 0x00002   # Graphics mode switch (2)
    ...
    def test_fan_control_acpi(self) -> Dict[str, Any]:
        ...
        for mode_name, mode_value in self.FAN_MODES.items():
            print(f"Testing fan mode: {mode_name} (value: {mode_value})")

            # Create packet with fan mode data
            data = struct.pack('B', mode_value)
            packet = self.create_bios_packet(self.CMD_DEFAULT, data)
    ...
    def set_fan_mode(self, mode: str) -> bool:
        """Set fan mode using ACPI method"""
        ...
        print(f"Setting fan mode to: {mode} (value: {mode_value})")
